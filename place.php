...
// 1. ดึงข้อมูลสถานที่ทั้งหมดมาก่อน
$sql = "SELECT * FROM nearby_place ORDER BY place_id DESC";
...
// -------------------------------------------------------------------------
// 2. ดึงรูปภาพทั้งหมด และ "กรองเฉพาะรูปที่มีไฟล์อยู่จริงในโฟลเดอร์" เท่านั้น!
$sql_all_images = "SELECT place_id, file_path FROM nearby_place_image ORDER BY image_id ASC";
...
if ($res_all_images) {
    while ($img = $res_all_images->fetch_assoc()) {
        ...
        // เช็คความชัวร์ว่าไฟล์นี้มีอยู่จริงในโฟลเดอร์ Place/
        if (file_exists("Place/" . $filepath)) {
            if (!isset($images_by_place[$pid])) {
                $images_by_place[$pid] = [];
            }
            $images_by_place[$pid][] = $filepath;
        }
    }
}

// 3. นำรูปแรกที่ใช้งานได้ ไปตั้งเป็น "รูปหน้าปก (Cover)" ให้แต่ละสถานที่
foreach ($all_places as $key => $place) {
    $pid = $place['place_id'];
    if (!empty($images_by_place[$pid])) {
        $all_places[$key]['image'] = $images_by_place[$pid][0];
    } else {
        $all_places[$key]['image'] = 'default.jpg';
    }
}
// -------------------------------------------------------------------------

// 4. ดึงรีวิวทั้งหมดและชื่อผู้ใช้ (ใช้ LEFT JOIN ป้องกันรีวิวหาย)
$sql_reviews = "
    SELECT r.*, COALESCE(a.username, 'ผู้ใช้ทั่วไป') AS username 
    FROM nearby_place_reviews r 
...
// จัดกลุ่มรีวิวตาม place_id เพื่อให้ JavaScript ดึงไปใช้ง่ายๆ
$reviews_by_place = [];
foreach ($all_reviews as $review) {
    $pid = $review['place_id'];
    if (!isset($reviews_by_place[$pid])) {
        $reviews_by_place[$pid] = [];
    }
    $reviews_by_place[$pid][] = $review;
}

// ดึงประเภทสถานที่สำหรับทำปุ่ม Filter
...
<head>
    <meta charset="UTF-8">
    <title>Discover Places - AR Ganesha</title>
...
<body>
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="mainNavbar">
                <ul class="navbar-nav ms-auto text-center align-items-lg-center">
                    ...
                    <li class="nav-item"><a class="nav-link text-warning" href="place.php">Place</a></li>
    ...
    <section class="hero-banner p-0 m-0 rounded-0">
        <div class="banner-grid p-0 m-0 rounded-0">
            <img src="image/travel_banner.jpg" alt="Banner Image" class="img-fluid w-100 rounded-0"
    ...
    <div style="margin-top: 70px;" class="container pb-5">
        ...
        <div class="mb-4 text-center">
            <h1 class="page-title">Discover Places</h1>
            <p style="color: #999;">แนะนำสถานที่ท่องเที่ยวที่น่าสนใจ</p>
        </div>

        <div class="d-flex justify-content-center gap-3 mb-5 overflow-auto pb-2" style="scrollbar-width: none;">
            <button class="filter-btn active" onclick="filterCategory('all', this)">All Categories</button>
            <?php foreach ($place_categories as $cat): ?>
                <button class="filter-btn" onclick="filterCategory('<?= htmlspecialchars($cat) ?>', this)">
        ...
        <div id="no-data-message" class="text-center w-100 py-5" style="display: none;">
            <i class="fa-solid fa-map-location-dot fs-1 mb-3" style="color: #555;"></i>
            <h4 class="text-white fw-bold">ยังไม่มีสถานที่แนะนำในหมวดหมู่นี้</h4>
            <p style="color: #999;">ลองเลือกดูหมวดหมู่ที่น่าสนใจอื่นๆ ด้านบนแทนนะครับ</p>
        </div>

        <div class="row g-4" id="place-grid">
            <?php foreach ($all_places as $res):
                ...
                $p_reviews = $reviews_by_place[$place_id] ?? [];
                $review_count = count($p_reviews);
                $total_score = 0;

                if ($review_count > 0) {
                    foreach ($p_reviews as $r) {
                ...
                $place_reviews_json = json_encode($p_reviews, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE);
                $place_images_json = json_encode($images_by_place[$place_id] ?? [], JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE);
            ?>
                <!-- ใส่ data-category ทั้งก้อนซ่อนไว้ให้ JS ค้นหา -->
                <div class="col-md-4 place-item" data-category="<?= htmlspecialchars($res['category'] ?? 'ไม่ระบุ') ?>" style="display: none;">

                    <div class="card place-card h-100">
                        <div class="position-relative">
                            <img src="Place/<?= htmlspecialchars($res['image']) ?>" class="card-img-top w-100" onerror="this.onerror=null; this.src='image/default.jpg';">
                            <div class="rating-badge"><i class="fa fa-star me-1"></i> <?= $avg_rating ?></div>
                        </div>

                        <div class="card-body d-flex flex-column p-4">
                            <div class="d-flex justify-content-between align-items-start mb-1">
                                <h5 class="fw-bold mb-0 text-truncate" style="color: #ffffff; max-width: 65%;" title="<?= htmlspecialchars($res['name']) ?>">
                            ...
                            <!-- หั่น String ด้วยลูกน้ำเพื่อทำป้าย Badge หลายๆอัน -->
                            <div class="d-flex flex-wrap gap-1 mb-3 mt-2">
                                ...
                                foreach ($cats as $c):
                                    if (trim($c) != ''):
                                ?>
                                        <span class="cat-chip"><i class="fa-solid fa-location-dot me-1"></i> <?= htmlspecialchars(trim($c)) ?></span>
                                <?php
                                    endif;
                                endforeach;
                                ?>
                            ...
                            <button type="button" class="btn-view mt-3"
                                ...
                                data-phone="<?= htmlspecialchars($res['phone_number'] ?? 'ไม่ระบุ') ?>"
                                ...
                                data-images="<?= htmlspecialchars($place_images_json, ENT_QUOTES, 'UTF-8') ?>"
                                data-detail="<?= htmlspecialchars($res['detail'] ?? 'ไม่มีรายละเอียด') ?>"
                                data-reviews="<?= htmlspecialchars($place_reviews_json, ENT_QUOTES, 'UTF-8') ?>">
                                View Details & Review
                            </button>
                        </div>
                    </div>
        ...
        <div class="text-center mt-5" id="loadMoreContainer" style="display: none;">
            <button id="loadMorePlaceBtn" class="btn text-dark fw-bold px-4 py-2" style="background-color: #c9a03c; border-radius: 25px; transition: 0.3s;">
                ดูเพิ่มเติม
            </button>
        </div>
    ...
    <div class="modal fade" id="placeDetailModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content" style="background-color: #111; border: 1px solid #333; border-radius: 20px; overflow: hidden;">
                ...
                <div class="modal-header border-0 pb-0 position-absolute w-100 z-3" style="top: 0;">
                    <button type="button" class="btn-close btn-close-white bg-dark p-2 m-3 rounded-circle" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                ...
                <div class="modal-body p-0">
                    ...
                    <div class="p-4 p-md-5 text-white">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <h2 id="modal-name" class="fw-bold" style="color: #D4AF37;">ชื่อสถานที่</h2>
                        ...
                        <div class="d-flex flex-wrap gap-2 mb-4 mt-3">
                            ...
                            <!-- เพิ่มป้ายเบอร์โทรศัพท์ตรงนี้ -->
                            <span class="badge" style="background-color: #222; font-size: 0.9rem; padding: 10px 15px; border: 1px solid #444;">
                                <i class="fa-solid fa-phone text-success me-2"></i><span id="modal-phone">ไม่ระบุ</span>
                            </span>
                        ...
                        <h5 class="fw-bold mb-3 border-bottom border-secondary pb-2">รายละเอียด</h5>
                        ...
                        <div id="modal-reviews-container" class="review-scroll pe-2 mb-4" style="max-height: 300px; overflow-y: auto;">
                        </div>
                        ...
                        <div class="p-4 rounded-4" style="background-color: #1a1a1a; border: 1px solid #333;">
                            ...
                            <?php if (isset($_SESSION['id_account'])): ?>
                                <form action="save_place_review.php" method="POST" class="review-form">
                                    <input type="hidden" name="place_id" id="modal-place-id" value="">
                            ...
                            <?php else: ?>
                                <div class="text-center p-3">
                                    ...
                                    <a href="Backend/login.php?redirect=../place.php" class="btn btn-sm btn-outline-warning rounded-pill mt-3 px-4">
                                        เข้าสู่ระบบ
                                    </a>
                                </div>
                            <?php endif; ?>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function updateDisplay() {
                ...
                placeItems.forEach(item => {
                    ...
                    // เปลี่ยนจาก === เป็น .includes() เพื่อรองรับหลายหมวดหมู่
            ...
            if (loadMorePlaceBtn) {
                loadMorePlaceBtn.addEventListener("click", function() {
            ...
            if (placeDetailModal) {
                placeDetailModal.addEventListener('show.bs.modal', function(event) {
                    ...
                    const phone = button.getAttribute('data-phone'); // รับค่าเบอร์โทร
                    ...
                    placeDetailModal.querySelector('#modal-name').textContent = name;
                    placeDetailModal.querySelector('#modal-time').innerHTML = `<span style="color: ${statusColor}; font-weight: bold;">${statusText}</span> (${time})`;
                    placeDetailModal.querySelector('#modal-detail').textContent = detail;
                    placeDetailModal.querySelector('#modal-rating').textContent = rating;

                    // แสดงผลเบอร์โทรศัพท์
                    const modalPhone = placeDetailModal.querySelector('#modal-phone');
                    if (phone && phone.trim() !== '') {
                        modalPhone.textContent = phone;
                    } else {
                        modalPhone.textContent = 'ไม่ระบุ';
                    }

                    const modalCatContainer = placeDetailModal.querySelector('#modal-category-container');
                    modalCatContainer.innerHTML = '';
                    category.split(',').forEach(c => {
                        if (c.trim() !== '') {
                            modalCatContainer.innerHTML += `<span class="badge" style="background-color: #222; font-size: 0.9rem; padding: 10px 15px; border: 1px solid #444;"><i class="fa-solid fa-location-dot text-danger me-2"></i>${escapeHtml(c.trim())}</span>`;
                        }
                    });

                    const mapLink = placeDetailModal.querySelector('#modal-map-link');
                    if (mapUrl && mapUrl !== '') {
                        mapLink.href = mapUrl;
                    ...
                    const carouselIndicators = placeDetailModal.querySelector('#modal-carousel-indicators');
                    const prevBtn = placeDetailModal.querySelector('#modal-carousel-prev');
                    const nextBtn = placeDetailModal.querySelector('#modal-carousel-next');
                        ...
                        if (images.length === 0) {
                            carouselInner.innerHTML = `
                                <div class="carousel-item active h-100">
                                    <img src="Place/default.jpg" class="d-block w-100 h-100" style="object-fit: cover;">
                                </div>
                            `;
                            ...
                            images.forEach((imgPath, index) => {
                                ...
                                carouselInner.innerHTML += `
                                    <div class="carousel-item ${isActive} h-100">
                                        <img src="Place/${escapeHtml(imgPath)}" onerror="this.src='Place/default.jpg';" class="d-block w-100 h-100" style="object-fit: cover;">
                                    </div>
                                `;
                            });
                    ...
                    const reviewsContainer = placeDetailModal.querySelector('#modal-reviews-container');
                    reviewsContainer.innerHTML = '';

                    try {
                        const reviews = JSON.parse(reviewsJson || '[]');
                        if (reviews.length === 0) {
                            reviewsContainer.innerHTML = '<div class="text-center text-secondary py-3" style="background-color: #1a1a1a; border-radius: 10px; border: 1px dashed #333;"><i class="fa-regular fa-comment-dots fs-3 mb-2"></i><br>ยังไม่มีรีวิวสำหรับสถานที่นี้ มารีวิวเป็นคนแรกสิ!</div>';
                        } else {
